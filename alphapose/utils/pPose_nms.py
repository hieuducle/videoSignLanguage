...
def write_json(all_results, outputpath, form=None, for_eval=False, outputfile='alphapose-results.json'):
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            if form == 'cmu': # the form of CMU-Pose
                ...
                # Create temporary dictionaries for each keypoint category
                tmp_pose = {'pose_keypoints_2d': []}
                tmp_face = {'face_keypoints_2d': []}
                tmp_hand_left = {'hand_left_keypoints_2d': []}
                tmp_hand_right = {'hand_right_keypoints_2d': []}

                # Define the new order for pose keypoints
                new_order = [
                    0, 18, 6, 8, 10, 5, 7, 9, 19, 12, 14, 16, 11, 13, 15,
                    2, 1, 4, 3, 20, 22, 24, 21, 23, 25
                ]

                # Add body keypoints in the new order, skipping keypoint 17 (head)
                for i in new_order:
                # for i in range(26):
                    if i == 17:
                        continue  # Skip keypoint 17 (Head)
                    tmp_pose['pose_keypoints_2d'].append(result['keypoints'][i * 3])
                    tmp_pose['pose_keypoints_2d'].append(result['keypoints'][i * 3 + 1])
                    tmp_pose['pose_keypoints_2d'].append(result['keypoints'][i * 3 + 2])

                # Face keypoints (26-93)
                for i in range(26, 94):
                    tmp_face['face_keypoints_2d'].append(result['keypoints'][i * 3])
                    tmp_face['face_keypoints_2d'].append(result['keypoints'][i * 3 + 1])
                    tmp_face['face_keypoints_2d'].append(result['keypoints'][i * 3 + 2])
                    

                # Left hand keypoints (94-114)
                for i in range(94, 115):
                    tmp_hand_left['hand_left_keypoints_2d'].append(result['keypoints'][i * 3])
                    tmp_hand_left['hand_left_keypoints_2d'].append(result['keypoints'][i * 3 + 1])
                    tmp_hand_left['hand_left_keypoints_2d'].append(result['keypoints'][i * 3 + 2])

                # Right hand keypoints (115-135)
                for i in range(115, 136):
                    tmp_hand_right['hand_right_keypoints_2d'].append(result['keypoints'][i * 3])
                    tmp_hand_right['hand_right_keypoints_2d'].append(result['keypoints'][i * 3 + 1])
                    tmp_hand_right['hand_right_keypoints_2d'].append(result['keypoints'][i * 3 + 2])

                # Combine all keypoints into a single dictionary
                tmp = {
                    'pose_keypoints_2d': tmp_pose['pose_keypoints_2d'],
                    'face_keypoints_2d': tmp_face['face_keypoints_2d'],
                    'hand_left_keypoints_2d': tmp_hand_left['hand_left_keypoints_2d'],
                    'hand_right_keypoints_2d': tmp_hand_right['hand_right_keypoints_2d'],
                }
    ...
    # elif form == 'open': # the form of OpenPose
    # # Lưu tệp JSON chứa tất cả kết quả
    #     # video_name = "D0001B"
    #     print("nhay vao luu file json\n")
    #     video_name = osp.splitext(osp.split(outputpath)[1])[0]
    #     with open(os.path.join(outputpath, outputfile), 'w') as json_file:
    #         json_file.write(json.dumps(json_results_cmu))
            
    #     # if not os.path.exists(os.path.join(outputpath, video_name)):
    #     #     os.mkdir(os.path.join(outputpath, video_name))
    #     if not os.path.exists(os.path.join(outputpath)):
    #         os.mkdir(os.path.join(outputpath))
            
    #     for i, name in enumerate(json_results_cmu.keys()):
    #         new_name = f"{video_name}_{str(i).zfill(12)}_keypoints.json"
    #         with open(os.path.join(outputpath, new_name), 'w') as json_file:
    #             json_file.write(json.dumps(json_results_cmu[name]))
        ...
        # Kiểm tra xem tất cả các tệp đã tồn tại chưa
        for i, _ in enumerate(json_results_cmu.keys()):
            new_name = f"{video_name}_{str(i).zfill(12)}_keypoints.json"
            if not osp.exists(osp.join(outputpath, new_name)):
                all_files_exist = False
                break

        # Nếu tất cả các tệp đã tồn tại, không thực hiện ghi đè
        if not all_files_exist:
            print("nhay vao luu file json\n")
            # Tạo thư mục nếu không tồn tại
            if not osp.exists(osp.join(outputpath)):
                os.makedirs(osp.join(outputpath))

            # Ghi tất cả các tệp keypoints vào thư mục đích
            for i, name in enumerate(json_results_cmu.keys()):
                new_name = f"{video_name}_{str(i).zfill(12)}_keypoints.json"
                with open(os.path.join(outputpath, new_name), 'w') as json_file:
                    json_file.write(json.dumps(json_results_cmu[name]))
        else:
            print("Tất cả các tệp keypoints đã tồn tại, không ghi đè.")
